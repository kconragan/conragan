...
# A script to create a new link post for an Astro.js content collection.

# --- CONFIGURATION ---
# Set the directory where your links are stored.
# The script will create subdirectories for the year.
OUTPUT_DIR="src/content/links"
...
echo "Creating a new link post..."

# --- 1. GATHER LINK DETAILS ---
link_title=$(prompt_input "Link Title" "true")
external_url=$(prompt_input "External URL" "true")
link_tags_raw=$(prompt_input "Tags (comma-separated, optional)" "false")

# --- 2. GET USER CONTENT FROM EDITOR ---
temp_file=$(mktemp)
echo "Opening your default editor ($EDITOR). Write your commentary (body), then save and quit."
echo "---"
sleep 1
...
IFS=',' read -ra tags_array <<<"$link_tags_raw"
yaml_tags=""
...
# --- 5. FINAL OUTPUT ---
echo ""
echo "✅ Success! New link post created at:"
echo "$output_file"